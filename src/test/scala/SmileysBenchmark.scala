     ...
     performance of "Smileys" config (
       ...
       def setupParser() = (_ : String) => {
         parsers getOrElseUpdate(42, {
           ...
           val P = new SortIdentifier("P")

           S.setProductions(production(), production(LETTER), production(SPACE), production(COLON), production(COLON, P), production(LPAREN, S, RPAREN), production(S, S))
           P.setProductions(production(LPAREN), production(RPAREN))
